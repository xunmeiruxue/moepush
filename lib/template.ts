...
// 将日期转换为指定时区
function convertToTimezone(date: Date, timezone?: number | string): Date {
  if (timezone === undefined) return date
  ...
  try {
    if (typeof timezone === 'number') {
      ...
      return new Date(date.getTime() + diffOffset * 60 * 1000)
    } else {
      // 保持对字符串时区的支持
...
const BUILT_IN_FUNCTIONS = {
  ...
  // 格式化日期,将传入的日期字符串转换为指定格式和时区
  formatDate: (dateStr: string, format = 'YYYY-MM-DD HH:mm:ss', timezone?: number | string) => {
    if (!dateStr) return ''
    ...
    try {
      ...
      return formatDateTime(targetDate, format)
    } catch (error: any) {
      console.warn(`日期格式化错误: ${error.message}`)
      return dateStr
    }
  },
